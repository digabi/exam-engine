...
const AnnotatableFormula = React.memo(function AnnotatableFormula(props: Props) {
  ...
  const onClick = useCallback(() => {
    ...
    setNewAnnotation({
      annotationParts: [{ annotationAnchor: path, selectedText: assistiveTitle, startIndex: 0, length: 0 }],
      displayNumber,
      selectedText: assistiveTitle,
      hidden: false
    })
  }, [path])
  ...
  return (
    <>
      <span
        ...
        data-testid={path}
        data-annotation-content={assistiveTitle}
        onClick={onClick}
...
function useElementAttributes(props: Props) {
  ...
  const assistiveTitle = (getAttribute(props.element, 'assistive-title') || props.element.textContent?.trim()) ?? ''
  const isDisplayFormula = getAttribute(props.element, 'mode') === 'display'
  const className = classNames('e-formula e-annotable', { 'e-block e-text-center': isDisplayFormula }, props.className)

  return useMemo(() => ({ svg, assistiveTitle, className }), [svg, assistiveTitle, className])
}

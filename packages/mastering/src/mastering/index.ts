...
function addGradingInstructionAttributes(root: Element, language: string) {
  ...
  if (exam.examGradingInstruction) {
    const localization = exam.examGradingInstruction.localizations.find(
      lo => lo.element.attr('lang')?.name() == language
    )
    addPathAttribute(localization?.element ?? exam.examGradingInstruction.element)
  }
  for (const answer of exam.answers) {
    if (answer.gradingInstruction) {
      const localization = answer.gradingInstruction.localizations.find(
        lo => lo.element.attr('lang')?.value() == language
      )
      addPathAttribute(localization?.element ?? answer.gradingInstruction.element)
    }
  }
...
function parseGradingInstruction(gradingInstruction: Element): GradingInstruction {
  const localizations = gradingInstruction.find<Element>('.//e:localization', ns).map(localization => ({
